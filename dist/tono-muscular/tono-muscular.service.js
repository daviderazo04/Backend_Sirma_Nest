"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.TonoMuscularService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const tono_muscular_entity_1 = require("./entities/tono-muscular.entity");
const fisioterapia_service_1 = require("../fisioterapia/fisioterapia.service");
let TonoMuscularService = class TonoMuscularService {
    tonomuscularRepository;
    fisioterapiaService;
    constructor(tonomuscularRepository, fisioterapiaService) {
        this.tonomuscularRepository = tonomuscularRepository;
        this.fisioterapiaService = fisioterapiaService;
    }
    async create(createTonoMuscularDto) {
        const { idfisioterapia, ...restOfDto } = createTonoMuscularDto;
        const fisioterapia = await this.fisioterapiaService.findOne(idfisioterapia);
        if (!fisioterapia) {
            throw new common_1.NotFoundException(`Registro de Fisioterapia con ID ${idfisioterapia} no encontrado.`);
        }
        const existingTonomuscular = await this.tonomuscularRepository.findOne({
            where: { idfisioterapia: idfisioterapia },
        });
        if (existingTonomuscular) {
            throw new common_1.ConflictException(`Ya existe un registro de Tono Muscular para la Fisioterapia con ID ${idfisioterapia}. Use PATCH para actualizarlo.`);
        }
        const tonomuscular = this.tonomuscularRepository.create({
            idfisioterapia: idfisioterapia,
            idfisioterapia2: fisioterapia,
            ...restOfDto,
        });
        return await this.tonomuscularRepository.save(tonomuscular);
    }
    async findAll() {
        return await this.tonomuscularRepository.find({
            relations: ['idfisioterapia2'],
        });
    }
    async findOne(idfisioterapia) {
        const tonomuscular = await this.tonomuscularRepository.findOne({
            where: { idfisioterapia: idfisioterapia },
            relations: ['idfisioterapia2'],
        });
        if (!tonomuscular) {
            throw new common_1.NotFoundException(`Registro de Tono Muscular para Fisioterapia con ID ${idfisioterapia} no encontrado.`);
        }
        return tonomuscular;
    }
    async update(idfisioterapia, updateTonoMuscularDto) {
        const tonomuscular = await this.tonomuscularRepository.findOne({ where: { idfisioterapia: idfisioterapia } });
        if (!tonomuscular) {
            throw new common_1.NotFoundException(`Registro de Tono Muscular para Fisioterapia con ID ${idfisioterapia} no encontrado.`);
        }
        const { idfisioterapia: dtoIdfisioterapia, ...restOfDto } = updateTonoMuscularDto;
        Object.assign(tonomuscular, restOfDto);
        return await this.tonomuscularRepository.save(tonomuscular);
    }
    async remove(idfisioterapia) {
        const result = await this.tonomuscularRepository.delete(idfisioterapia);
        if (result.affected === 0) {
            throw new common_1.NotFoundException(`Registro de Tono Muscular para Fisioterapia con ID ${idfisioterapia} no encontrado.`);
        }
    }
};
exports.TonoMuscularService = TonoMuscularService;
exports.TonoMuscularService = TonoMuscularService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(tono_muscular_entity_1.Tonomuscular)),
    __param(1, (0, common_1.Inject)((0, common_1.forwardRef)(() => fisioterapia_service_1.FisioterapiaService))),
    __metadata("design:paramtypes", [typeorm_2.Repository,
        fisioterapia_service_1.FisioterapiaService])
], TonoMuscularService);
//# sourceMappingURL=tono-muscular.service.js.map